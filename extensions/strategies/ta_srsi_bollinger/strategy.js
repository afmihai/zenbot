...
const ta_srsi = require('../../../lib/ta_stochrsi')
const ta_bollinger = require('../../../lib/ta_bollinger')
...
module.exports = {
  ...
  getOptions: function () {
    ...
    this.option(
      'srsi_k_sell',
      'K must be above this before selling',
      Number,
      60
    )
    ...
    this.option(
      'srsi_dType',
      'D type mode : SMA,EMA,WMA,DEMA,TEMA,TRIMA,KAMA,MAMA,T3',
      String,
      'SMA'
    ),
    ...
    this.option(
      'bollinger_dType',
      'mode: : SMA,EMA,WMA,DEMA,TEMA,TRIMA,KAMA,MAMA,T3',
      String,
      'SMA'
    )
    this.option(
    ...
    ta_bollinger(
      ...
      .then(function (inbol) {
        ta_srsi(
          s,
          'srsi',
          s.options.srsi_periods,
          s.options.srsi_k,
          s.options.srsi_d,
          s.options.srsi_dType
        )
          .then(function (inres) {
            if (!inres) return cb()
            const divergent =
              inres.outFastK[inres.outFastK.length - 1] -
              inres.outFastD[inres.outFastD.length - 1]
            s.period.srsi_D = inres.outFastD[inres.outFastD.length - 1]
            s.period.srsi_K = inres.outFastK[inres.outFastK.length - 1]
            const last_divergent =
              inres.outFastK[inres.outFastK.length - 2] -
              inres.outFastD[inres.outFastD.length - 2]
            let _switch = 0 //s.lookback[0]._switch
            const nextdivergent =
              (divergent + last_divergent) / 2 + (divergent - last_divergent)
            ...
            let upperBound =
              inbol.outRealUpperBand[inbol.outRealUpperBand.length - 1]
            let lowerBound =
              inbol.outRealLowerBand[inbol.outRealLowerBand.length - 1]
            let midBound =
              inbol.outRealMiddleBand[inbol.outRealMiddleBand.length - 1]
  ...
  onReport: function (s) {
    ...
    if (s.period.bollinger) {
      if (s.period.bollinger.upperBound && s.period.bollinger.lowerBound) {
        ...
        cols.push(
          z(8, n(s.period.srsi_D).format('0.0000').substring(0, 7), ' ').cyan
        )
        cols.push(
          z(8, n(s.period.srsi_K).format('0.0000').substring(0, 7), ' ').cyan
        )
        cols.push(
          z(5, n(s.period.divergent).format('0').substring(0, 7), ' ').cyan
        )
        cols.push(
          z(2, n(s.period._switch).format('0').substring(0, 2), ' ').cyan
        )
      }
    } else {
      cols.push('         ')
  ...
  phenotypes: {
    // -- common
    period_length: Phenotypes.ListOption([
      '1m',
      '2m',
      '3m',
      '4m',
    ...
    srsi_dType: Phenotypes.ListOption([
      'SMA',
      'EMA',
      'WMA',
      'DEMA',
    ...
    bollinger_dType: Phenotypes.ListOption([
      'SMA',
      'EMA',
      'WMA',
      'DEMA',
