  ...
  onPeriod: function (s, cb) {
    if (s.period.kc) {
      if (s.period.kc.upper && s.period.kc.lower) {
        ...
        if (
          s.period.close >
          (upperChannel / 100) * (100 - s.options.kc_upper_channel_pct)
        ) {
          s.signal = 'sell'
        } else if (
          s.period.close <
          (lowerChannel / 100) * (100 + s.options.kc_lower_channel_pct)
        ) {
          s.signal = 'buy'
        } else {
          s.signal = null // hold
  ...
  onReport: function (s) {
    ...
    if (s.period.kc) {
      if (s.period.kc.upper && s.period.kc.lower) {
        ...
        cols.push(
          z(8, n(lowerChannel).format('0.000000').substring(0, 7), ' ').cyan
        )
        cols.push(
          z(8, n(upperChannel).format('0.000000').substring(0, 7), ' ').cyan
        )
      }
    } else {
      cols.push('         ')
