...
module.exports = {
  ...
  getOptions: function () {
    ...
    this.option(
      'tenkenSenPeriods',
      'Tenkan-sen (Conversion Line) Periods',
      Number,
      9
    ) //default 9
    ...
    this.option(
      'kumoBreakoutWeight',
      'Kumo Breakout Importance Weight',
      Number,
      1
    ) //range 0 - 2 Default = 1
    this.option(
      'senkouCrossWeight',
      'Senkou (Leading) Span Cross Importance Weight',
      Number,
      1
    ) //range 0 - 2 Default = 1
    this.option(
      'chikouCrossWeight',
      'Chikou (Lagging) Span Cross Importance Weight',
      Number,
      1
    ) //range 0 - 2 Default = 1
    this.option(
      'chikouPlacementWeight',
      'Chikou (Lagging) Span Relative to Cloud Importance Weight',
      Number,
      1
    ) //range 0 - 2 Default = 1
    this.option(
      'pricePlacementWeight',
      'Price Relative to Cloud Importance Weight',
      Number,
      1
    ) //range 0 - 2 Default = 1
  ...
  calculate: function (s) {
    if (s.lookback.length > s.options.min_periods) {
      ...
      if (s.lookback.length > s.options.displacement) {
        ...
        s.priceAboveKumo = valueAbove(
          s.period.close,
          s.period.senkouA,
          s.period.senkouB
        )
        s.priceBelowKumo = valueBelow(
          s.period.close,
          s.period.senkouA,
          s.period.senkouB
        )
        ...
        s.period.kumoBreakoutScore = calcKumoBreakout(
          s,
          s.lookback[0].kumoBreakoutScore
        )
        s.period.senkouCrossScore = calcSenkouCross(
          s,
          s.lookback[0].senkouCrossScore
        )
        s.period.chikouCrossScore = calcChikouCross(
          s,
          s.lookback[0].chikouCrossScore
        )
        s.period.pricePlacementScore = calcPricePlacement(
          s,
          s.lookback[0].pricePlacementScore
        )
        s.period.chikouPlacementScore = calcChikouPlacement(
          s,
          s.lookback[0].chikouPlacementScore
        )

        s.totalScore = s.options.tkCrossWeight * s.period.tkCrossScore
        s.totalScore += s.options.pkCrossWeight * s.period.pkCrossScore
        s.totalScore +=
          s.options.kumoBreakoutWeight * s.period.kumoBreakoutScore
        ...
        s.totalScore +=
          s.options.pricePlacementWeight * s.period.pricePlacementScore
        s.totalScore +=
          s.options.chikouPlacementWeight * s.period.chikouPlacementScore
  ...
  onPeriod: function (s, cb) {
    ...
    if (s.options.debug) {
      console.log(
        s.normalizedScore > s.options.buyLevel &&
          s.previousScore < s.options.buyLevel
      )
    }
    ...
    if (
      ...
      s.normalizedScore > s.options.sellLevel &&
      s.previousScore > s.options.buyLevel &&
      s.previousScore < s.options.sellLevel
    ) {
      s.signal = 'buy'
      s.previousScore = s.normalizedScore
    } else if (
  ...
  onReport: function (s) {
    ...
    cols.push(
      z(10, 'S[' + n(s.normalizedScore).format('###.0') + ']', '')[color]
    )
...
function getIntersect(s, key1, key2) {
  return (
    ...
    (s.period[key2] -
      s.lookback[0][key2] -
      (s.period[key1] - s.lookback[0][key1]))
  )
}
...
function valueBelowKumo(s, val, key1, key2) {
  if (s.lookback.length >= s.options.displacement)
    return valueBelow(
      val,
      s.lookback[s.options.displacement][key1],
      s.lookback[s.options.displacement][key2]
    )
...
function calcKumoBreakout(s, previousVal) {
  let bullish =
    (crossover(s, 'close', 'senkouA') &&
      s.period.senkouA >= s.period.senkouB) ||
    (crossover(s, 'close', 'senkouB') && s.senkouB >= s.senkouA)
  let bearish =
    (crossunder(s, 'close', 'senkouB') &&
      s.period.senkouA >= s.period.senkouB) ||
