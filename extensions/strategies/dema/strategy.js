...
module.exports = {
  ...
  getOptions: function () {
    ...
    this.option(
      'noise_level_pct',
      'do not trade when short ema is with this % of last short ema',
      Number,
      0
    )
  },
  ...
  onPeriod: function (s, cb) {
    ...
    // TODO: Check types
    if (
      typeof s.period.dema_histogram === 'number' &&
      typeof s.lookback[0].dema_histogram === 'number'
    ) {
      if (
        s.options.noise_level_pct != 0 &&
        (s.period.ema_short / s.lookback[0].ema_short) * 100 <
          s.options.noise_level_pct
      ) {
        s.signal = null
      } else if (
