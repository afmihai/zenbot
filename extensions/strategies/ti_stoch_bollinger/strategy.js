...
module.exports = {
  ...
  onPeriod: function (s, cb) {
    ...
    ti_bollinger(s, 'ti_bollinger', s.options.bollinger_size, s.options.bollinger_time).then(function (inbol) {
      ti_stoch(s, 'ti_stoch', s.options.stoch_kperiods, s.options.stoch_k, s.options.stoch_d).then(function (inres) {
        ...
        if (_switch !== 0) {
          if (s.period.close >= MiddleBand && s.period.close >= ((UpperBand / 100) * (100 + s.options.bollinger_upper_bound_pct)) && nextdivergent < divergent && _switch === -1 && s.period.stoch_K > s.options.stoch_k_sell) {
            s.signal = 'sell'
          } else if (s.period.close < (LowerBand / 100) * (100 + s.options.bollinger_lower_bound_pct) && nextdivergent >= divergent && _switch === 1 && s.period.stoch_K < s.options.stoch_k_buy) {
            s.signal = 'buy'
          }
