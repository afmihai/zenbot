...
const tulip_stoch = require('../../../lib/ti_stoch')
...
module.exports = {
  ...
  onPeriod: function (s, cb) {
    ...
    tulip_stoch(
      ...
      .then(function (result) {
        ...
        const divergent =
          result.k[result.k.length - 1] - result.d[result.d.length - 1]
        s.period.srsi_D = result.d[result.d.length - 1]
        s.period.srsi_K = result.k[result.k.length - 1]
        const last_divergent =
          result.k[result.k.length - 2] - result.d[result.d.length - 2]
        ...
        if (_switch !== 0) {
          if (_switch === -1 && s.period.srsi_K > s.options.stoch_k_sell) {
            ...
            _switch === 1 &&
            s.period.srsi_K < s.options.stoch_k_buy
          ) {
            s.signal = 'buy'
          }
        }
  ...
  onReport: function (s) {
    ...
    cols.push(
      z(8, n(s.period.srsi_D).format('0.000000').substring(0, 7), ' ').cyan
    )
    cols.push(
      z(8, n(s.period.srsi_K).format('0.000000').substring(0, 7), ' ').cyan
    )
