...
module.exports = function gemini(conf) {
  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.gemini ||
        !conf.gemini.key ||
        !conf.gemini.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your Gemini credentials in ' +
  ...
  function joinProduct(product_id) {
    return (
      product_id.split('-')[0].toLowerCase() + product_id.split('-')[1]
    ).toLowerCase()
  }
  ...
  function retry(method, args, error) {
    if (error.code === 429) {
      console.error(
        `\nGemini API rate limit exceeded! unable to call ${method}, aborting`
          .red
      )
      return
    }
  ...
  var exchange = {
    ...
    getBalance: function (opts, cb) {
      ...
      client
        ...
        .then(body => {
          var asset = body.find(
            x => x.currency.toLowerCase() === opts.asset.toLowerCase()
          )
          var currency = body.find(
            x => x.currency.toLowerCase() === opts.currency.toLowerCase()
          )
          ...
          debugOut(
            `  ${currency.currency} (${balance.currency}/${balance.currency_hold})`
          )
          debugOut(
            `  ${asset.currency} (${balance.asset}/${balance.asset_hold})`
          )
