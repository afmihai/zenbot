...
module.exports = function container(conf) {
  ...
  function publicClient() {
    if (!public_client)
      public_client = new ccxt.therock({ apiKey: '', secret: '' })
  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.therock ||
        !conf.therock.key ||
        !conf.therock.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your TheRockTrading credentials in ' +
            path.resolve(__dirname, 'conf.js')
        )
      }

      authed_client = new ccxt.therock({
        apiKey: conf.therock.key,
        secret: conf.therock.secret,
      })
    }
  ...
  function retry(method, args) {
    if (method !== 'getTrades') {
      console.error(
        (
          '\n TheRockTrading API is down! unable to call ' +
          method +
          ', retrying in 10s'
        ).red
      )
        ...
        client
          ...
          .then(function (response) {
            ...
            if (
              ...
              response['meta'].current.page < response['meta'].next.page
            ) {
