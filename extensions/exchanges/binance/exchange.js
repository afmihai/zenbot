...
module.exports = function binance(conf) {
  const exchange = {
    ...
    getTrades: function (opts, cb) {
      ...
      client
        .fetchTrades(symbol, startTime, undefined, args)
        .then(result => {
          if (result.length === 0 && opts.from) {
            ...
            if (startTime + time_diff < new Date().getTime() - 3600000) {
              ...
              return client
                .fetchOHLCV(symbol, undefined, startTime)
                .then(ohlcv => {
                  return ohlcv.length
                    ? client.fetchTrades(symbol, ohlcv[0][0])
                    : []
                })
            }
          }
  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.binance ||
        !conf.binance.key ||
        conf.binance.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your Binance credentials in ' +
  ...
  function retry(method, args, err) {
    if (method !== 'getTrades') {
      console.error(
        (
          '\nBinance API is down! unable to call ' +
          method +
          ', retrying in 20s'
        ).red
      )
