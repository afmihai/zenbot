...
module.exports = function container(conf) {
  ...
  function publicClient() {
    if (!public_client)
      public_client = new ccxt.hitbtc({ apiKey: '', secret: '' })
  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.hitbtc ||
        !conf.hitbtc.key ||
        !conf.hitbtc.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your HitBTC credentials in ' +
            path.resolve(__dirname, 'conf.js')
        )
      }
      authed_client = new ccxt.hitbtc({
        apiKey: conf.hitbtc.key,
        secret: conf.hitbtc.secret,
      })
    }
  ...
  var exchange = {
    ...
    getOrder: function (opts, cb) {
      ...
      client
        .fetchOrder(opts.order_id, joinProduct(opts.product_id), {
          wait: 100000,
        })
        .then(result => {
          let r = result
          if (result.status === 'canceled') {
            ...
            return cb({
              ...
              desc: 'Order cancel or deleted',
            })
          }
          if (result.status == 'open') {
            result.status = 'open'
            result.filled_size =
              parseFloat(result.amount) - parseFloat(result.remaining)
