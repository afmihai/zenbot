  ...
  function authedClient() {
    if (!authed_client) {
      if (
        ...
        !conf.cexio.username ||
        !conf.cexio.key ||
        conf.cexio.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your CEX.IO credentials in ' +
            path.resolve(__dirname, 'conf.js')
        )
      }
      authed_client = new CEX(
        conf.cexio.username,
        conf.cexio.key,
        conf.cexio.secret
      ).rest
            ...
            case 'history-update':
              msg.data.forEach(function (trade) {
                ws_trades.push({
                  ...
                  time: Number(trade[1]),
                  size: Number(
                    n(trade[2]).divide(amount_format).format('0.00000000')
                  ),
        ...
        if (!ws_subscribed)
          wsTrades(opts.product_id)
            .then(function (data) {
              ws_subscribed = true
              amount_format =
                opts.product_id.split('-')[0] === 'ETH' ? 1000000 : 100000000 // trade amount is an unformatted integer
              data.forEach(function (trade) {
                ...
                ws_trades.push({
                  ...
                  time: Number(t[1]),
                  size: Number(
                    n(t[2]).divide(amount_format).format('0.00000000')
                  ),
                  price: Number(t[3]),
                  side: t[0],
                })
        ...
        client.place_order(
          ...
          function (err, body) {
            if (err || (typeof body === 'string' && body.match(/error/))) {
              ...
              if (
                body === 'error: Error: Place order error: Insufficient funds.'
              ) {
                let order = {
                  status: 'rejected',
    ...
    setFees: function (opts) {
      ...
      client.get_my_fee(function (err, body) {
        if (err || (typeof body === 'string' && body.match(/error/))) {
          ...
          let makerFee =
            (parseFloat(body[pair].buyMaker) +
              parseFloat(body[pair].sellMaker)) /
            2
          let takerFee =
            (parseFloat(body[pair].buy) + parseFloat(body[pair].sell)) / 2
