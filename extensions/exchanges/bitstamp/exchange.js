...
module.exports = function bitstamp(conf) {
  function authedClient() {
    if (conf.bitstamp.key && conf.bitstamp.key !== 'YOUR-API-KEY') {
      return new Bitstamp(
        conf.bitstamp.key,
        conf.bitstamp.secret,
        conf.bitstamp.client_id
      )
    }
    throw new Error(
      '\nPlease configure your Bitstamp credentials in ' +
  ...
  Bitstamp.prototype.tradeDaily = function (
    direction,
    market,
    amount,
    price,
  ...
  Bitstamp.prototype.tradeMarket = function (
    direction,
    market,
    amount,
    callback
  ...
  Bitstamp_WS.prototype.subscribe = function () {
    if (wsOpts.pairOk) {
      ...
      this.client.bind(
        wsOpts.trades.evType,
        this.broadcast(wsOpts.trades.evType)
      )
      this.client.subscribe(wsOpts.quotes.channel)
      this.client.bind(
        wsOpts.quotes.evType,
        this.broadcast(wsOpts.quotes.evType)
      )
    }
  }
  ...
  function retry(method, wait, args) {
    if (method !== 'getTrades') {
      console.error(
        (
          '\nBitstamp API is not answering! unable to call ' +
          method +
          ', retrying in ' +
          wait +
          's'
  ...
  var exchange = {
    ...
    getBalance: function (opts, cb) {
      ...
      client.balance(null, function (err, body) {
        ...
        balance.currency =
          n(body[opts.currency.toLowerCase() + '_balance']) + 0.000001
        balance.asset =
          n(body[opts.asset.toLowerCase() + '_balance']) + 0.000001
        balance.currency_hold =
          n(body[opts.currency.toLowerCase() + '_reserved']) + 0.000001
        balance.asset_hold =
          n(body[opts.asset.toLowerCase() + '_reserved']) + 0.000001

        if (
          typeof balance.asset == undefined ||
          typeof balance.currency == undefined
        ) {
    ...
    trade: function (type, opts, cb) {
      ...
      if (opts.order_type === 'maker') {
        client.tradeDaily(type, currencyPair, opts.size, opts.price, function (
          err,
          body
        ) {
          body = statusErr(err, body)
    ...
    getOrder: function (opts, cb) {
      ...
      client.order_status(opts.order_id, function (err, body) {
        ...
        if (body.status === 'done') {
          if (body.transactions && body.transactions[0].datetime)
            body.done_at = body.transactions[0].datetime
        }
