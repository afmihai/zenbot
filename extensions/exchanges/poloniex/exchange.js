  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.poloniex ||
        !conf.poloniex.key ||
        conf.poloniex.key === 'YOUR-API-KEY'
      ) {
  ...
  var exchange = {
    ...
    trade: function (type, opts, cb) {
      ...
      client._private(type, params, function (err, result) {
        ...
        if (
          result &&
          result.error === 'Unable to place post-only order at this price.'
        ) {
          ...
          result &&
          result.error &&
          result.error.match(/^Nonce must be greater/)
        ) {
          return retry('trade', args)
        }
        if (!err && result.error) {
