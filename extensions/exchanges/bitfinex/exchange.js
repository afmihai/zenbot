...
module.exports = function bitfinex(conf) {
      ...
      if (so.debug)
        console.warn(
          (
            '\nWarning: Order ' +
            cid +
            ' not found in cache for wsUpdateOrder (manual order?).'
          ).red
        )
      return
    }
      ...
      if (so.debug)
        console.warn(
          (
            '\nWarning: Order ' +
            cid +
            ' not found in cache for wsUpdateOrderCancel (manual order?).'
          ).red
        )
      return
    }
  ...
  function wsUpdateReqOrder(error) {
    if (
      error[6] === 'ERROR' &&
      error[7].match(/^Invalid order: not enough .* balance for/)
    ) {
      var cid = error[4][2]
        ...
        if (so.debug)
          console.warn(
            (
              '\nWarning: Order ' +
              cid +
              ' not found in cache for wsUpdateReqOrder (manual order?).'
            ).red
          )
        return
      }
  ...
  function updateWallet(wallets) {
    ...
    wallets.forEach(function (wallet) {
      if (wallet['type'] === conf.bitfinex.wallet) {
        ...
        ws_balance[wallet['currency'].toUpperCase()].available = wallet[
          'balanceAvailable'
        ]
  ...
  function wsClose() {
    ...
    console.error(
      '\nWebSockets Error: Connection closed.'.red +
        ' Retrying every ' +
  ...
  function wsError(e) {
    ...
    if (e.event == 'auth' && e.status == 'FAILED') {
      var errorMessage = (
        '\nWebSockets Warning: Authentication ' +
        e.status +
        ' (Reason: "' +
        e.msg +
        '").'
  ...
  function wsClient() {
    if (!ws_client) {
      if (
        !conf.bitfinex ||
        !conf.bitfinex.key ||
        conf.bitfinex.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your Bitfinex credentials in ' +
      ...
      ws_client = new BFX({
        apiKey: conf.bitfinex.key,
        apiSecret: conf.bitfinex.secret,
        transform: true,
      }).ws()
      ...
      ws_client.once('auth', () => {
        ...
        ws_client.onMarginInfoUpdate({}, symbol =>
          marginSymbolWebsocket(symbol)
        )
        ws_client.onPositionSnapshot({}, positions =>
          assetPositionMargin(positions)
        )
        ws_client.onNotification({ type: 'on-req' }, order =>
          wsUpdateReqOrder(order)
        )
      })
      ...
      case 'margin':
        try {
          ...
          if (so.debug) {
            console.warn(e)
            console.warn(
              "\nWebSockets Warning: Cannot send 'calc' for getBalance update (maybe connection not open?)."
                .red + ' Waiting for reconnect.'
            )
          }
        }
  ...
  var exchange = {
        ...
        setTimeout(function () {
          ...
          client.trades(
            ...
            function (err, body) {
              ...
              if (so.debug && trades.length > 0)
                console.log(
                  new Date().toISOString(),
                  'got trade count ',
                  trades.length,
                  ' range: ',
                  new Date(trades[trades.length - 1].time).toISOString(),
                  '-',
                  new Date(trades[0].time).toISOString()
                )
              cb(null, trades)
            }
          )
    ...
    getBalance: function (opts, cb) {
      ...
      if (
        ...
        (ws_walletCalcDone[opts.asset] === false &&
          ws_walletCalcDone[opts.currency] === true) ||
    ...
    cancelOrder: function (opts, cb) {
      ...
      try {
        ...
        if (so.debug) {
          console.warn(e)
          console.warn(
            '\nWebSockets Warning: Cannot send cancelOrder (maybe connection not open?).'
              .red +
              ' Retrying in ' +
              (ws_retry / 1000 + ' seconds').yellow +
              '.'
    ...
    trade: function (action, opts, cb) {
      ...
      let walletName =
        conf.bitfinex.wallet.toUpperCase() === 'EXCHANGE' ? 'EXCHANGE ' : ''
      ...
      try {
        ...
        if (so.debug) {
          console.warn(e)
          console.warn(
            '\nWebSockets Warning: Cannot send trade (maybe connection not open?).'
              .red +
    ...
    getOrder: function (opts, cb) {
      ...
      if (order.status === 'rejected' && order.reject_reason === 'post only') {
        ...
        order.status === 'rejected' &&
        order.reject_reason === 'zenbot canceled'
      ) {
        return cb(null, order)
      }
