...
module.exports = function container(conf) {
  ...
  function authedClient() {
    if (!authed_client) {
      if (
        !conf.quadriga ||
        !conf.quadriga.key ||
        !conf.quadriga.key === 'YOUR-API-KEY'
      ) {
        throw new Error(
          'please configure your Quadriga credentials in ' +
            path.resolve(__dirname, 'conf.js')
        )
      }

      authed_client = new QuadrigaCX(
        conf.quadriga.client_id,
        conf.quadriga.key,
        conf.quadriga.secret
      )
    }
  ...
  function retry(method, args, error) {
    if (error.code === 200) {
      console.error(
        `\nQuadrigaCX API rate limit exceeded! unable to call ${method}, aborting`
          .red
      )
      return
    }
  ...
  var exchange = {
    ...
    getTrades: function (opts, cb) {
      ...
      client.api('transactions', args, function (err, body) {
        if (!shownWarnings) {
          console.log(
            'please note: the quadriga api does not support backfilling.'
          )
          console.log(
            'please note: period lengths should be set to 1h or less.'
          )
          shownWarnings = true
        }
        ...
        var trades = body
          .filter(t => {
            return typeof opts.from === 'undefined'
              ? true
              : moment.unix(t.date).valueOf() > opts.from
          })
          .reverse()
          .map(function (trade) {
            return {
    ...
    getBalance: function (opts, cb) {
      ...
      client.api('balance', function (err, wallet) {
        ...
        balance.currency_hold = n(wallet[currency + '_reserved']).format(
          '0.00000000'
        )
        ...
        debugOut(
          `  ${currency} (${wallet[currency + '_balance']}/${
            wallet[currency + '_reserved']
          }/${wallet[currency + '_available']})`
        )
        debugOut(
          `  ${asset} (${wallet[asset + '_balance']}/${
            wallet[asset + '_reserved']
          }/${wallet[asset + '_available']})`
        )
        ...
        if (opts.order_type === 'taker') {
          ...
          if (body.orders_matched) {
            ...
            for (var idx = 0; idx < order_count; idx++) {
              asset_total =
                asset_total + Number(body.orders_matched[idx].amount)
              price_total =
                price_total +
                Number(body.orders_matched[idx].amount) *
                  Number(body.orders_matched[idx].price)
            }
