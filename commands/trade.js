...
module.exports = function (program, conf) {
  program
    ...
    .option(
      '--manual',
      'watch price and account balance, but do not perform trades automatically',
      Boolean,
      false
    )
    .option(
      '--reverse',
      'use this and all your signals(buy/sell) will be switch! TAKE CARE!',
      Boolean,
      false
    )
    .option(
      '--non_interactive',
      'disable keyboard inputs to the bot',
      Boolean,
      false
    )
    .option(
    ...
    .option(
      '--sell_cancel_pct <pct>',
      'cancels the sale if the price is between this percentage (for more or less)',
      Number,
      conf.sell_cancel_pct
    )
    .option(
      '--max_sell_loss_pct <pct>',
      'avoid selling at a loss pct under this float',
      conf.max_sell_loss_pct
    )
    .option(
      '--max_buy_loss_pct <pct>',
      'avoid buying at a loss pct over this float',
      conf.max_buy_loss_pct
    )
    .option(
    ...
    .option(
      '--currency_increment <amount>',
      'Currency increment, if different than the asset increment',
      String,
      null
    )
    .option(
    ...
    .option(
      '--use_prev_trades',
      'load and use previous trades for stop-order triggers and loss protection'
    )
    .option(
    ...
    .option(
      '--interval_trade <minutes>',
      'The interval trade time',
      Number,
      conf.interval_trade
    )
    .option(
      '--quarentine_time <minutes>',
      'For loss trade, set quarentine time for cancel buys',
      Number,
      conf.quarentine_time
    )
    ...
    .action(function (selector, cmd) {
      ...
      keyMap.set(
        'O',
        'show current trade options in a dirty view (full list)'.grey
      )
      ...
      function listOptions() {
        printLog(
          s.exchange.name.toUpperCase() +
            ' exchange active trading options:'.grey,
          true
        )
        printLog(
          z(22, 'STRATEGY'.grey, ' ') +
            ...
            so.strategy +
            '\t' +
            require(`../extensions/strategies/${so.strategy}/strategy`)
              .description.grey,
          true
        )
        printLog(
          ...
          [
            z(
              ...
              so.mode === 'paper'
                ? '      '
                : so.mode === 'live' &&
                  (so.manual === false || typeof so.manual === 'undefined')
                ? '       ' + 'AUTO'.black.bgRed + '    '
            ...
            z(
              29,
              so.order_type === 'maker'
                ? so.order_type.toUpperCase().green
                : so.order_type.toUpperCase().red,
              ' '
            ),
            z(
              31,
              so.mode === 'paper'
                ? 'avg. '.grey + so.avg_slippage_pct + '%'
                : 'max '.grey + so.max_slippage_pct + '%',
              ' '
            ),
            z(
              20,
      ...
      function printTrade(quit, dump, statsonly = false) {
        const tmp_balance = n(s.balance.currency)
        ...
        const profit = s.start_capital
          ? n(tmp_balance).subtract(s.start_capital).divide(s.start_capital)
          : n(0)
        const buy_hold = s.start_price
          ? n(s.period.close).multiply(n(s.start_capital).divide(s.start_price))
          : n(tmp_balance)
        const buy_hold_profit = s.start_capital
          ? n(buy_hold).subtract(s.start_capital).divide(s.start_capital)
          : n(0)
        if (!statsonly) {
          console.log()
          var output_lines = []
          output_lines.push(
            'last balance: ' +
              n(tmp_balance).format('0.00000000').yellow +
              ' (' +
              profit.format('0.00%') +
              ')'
          )
          output_lines.push(
            'buy hold: ' +
              buy_hold.format('0.00000000').yellow +
              ' (' +
              n(buy_hold_profit).format('0.00%') +
              ')'
          )
          output_lines.push(
            'vs. buy hold: ' +
              n(tmp_balance).subtract(buy_hold).divide(buy_hold).format('0.00%')
                .yellow
          )
          output_lines.push(
            s.my_trades.length +
        ...
        if (s.my_trades.length && sells > 0) {
          if (!statsonly) {
            ...
            output_lines.push(
              'error rate: ' +
                (sells ? n(losses).divide(sells).format('0.00%') : '0.00%')
                  .yellow
            )
          }
          ...
          s.stats.error_rate = sells
            ? n(losses).divide(sells).format('0.00%')
            : '0.00%'
        }
        if (!statsonly) {
        ...
        if (quit || dump) {
          const html_output = output_lines
            .map(function (line) {
          ...
          if (so.filename !== 'none') {
            let out_target
            const out_target_prefix = so.paper
              ? 'simulations/paper_result_'
              : 'stats/trade_result_'
            if (dump) {
      ...
      function saveStats() {
        ...
        if (s.my_trades.length && sells > 0) {
          ...
          output_lines.push(
            'error rate: ' +
              (sells ? n(losses).divide(sells).format('0.00%') : '0.00%').yellow
          )
        ...
        if (so.filename !== 'none') {
          ...
          //ymd
          const today = dt.slice(2, 4) + dt.slice(5, 7) + dt.slice(8, 10)
          let out_target_prefix = so.paper
            ? 'simulations/paper_result_'
            : 'stats/trade_result_'
          out_target =
            so.filename ||
        ...
        if (key === 'l') {
          ...
          printLog(
            'MANUAL trade in LIVE mode: ' +
              (so.manual ? 'ON'.green.inverse : 'OFF'.red.inverse),
            true
          )
          ...
          printLog(
            'DEBUG mode: ' +
              (debug.on ? 'ON'.green.inverse : 'OFF'.red.inverse),
            true
          )
          ...
          trades
            ...
            .toArray(function (err, trades) {
              ...
              if (!trades.length) {
                ...
                const minuses = Math.floor(
                  (head.length - so.mode.length - 19) / 2
                )
                console.log(
                  '-'.repeat(minuses) +
                    ' STARTING ' +
                      ...
                      if (prev_session && !cmd.reset_profit) {
                        if (
                          ...
                          ((so.mode === 'paper' &&
                            ...
                            !raw_opts.asset_capital) ||
                            (so.mode === 'live' &&
                              prev_session.balance.asset == s.balance.asset &&
                              prev_session.balance.currency ==
                                s.balance.currency))
                        ) {
                          s.orig_capital = session.orig_capital =
                            prev_session.orig_capital
                          s.orig_price = session.orig_price =
                            prev_session.orig_price
                          if (so.mode === 'paper') {
      ...
      function forwardScan() {
        function saveSession() {
          engine.syncBalance(function (err) {
            if (!err && s.balance.asset === undefined) {
              ...
              throw new Error(
                'Error during syncing balance. Please check your API-Key'
              )
            }
            if (err) {
              console.error(
            ...
            if (s.period) {
              ...
              b.consolidated = n(s.balance.asset)
                ...
                .value()
              b.profit =
                (b.consolidated - session.orig_capital) / session.orig_capital
              b.buy_hold =
                s.period.close * (session.orig_capital / session.orig_price)
              b.buy_hold_profit =
                (b.buy_hold - session.orig_capital) / session.orig_capital
              ...
              function (err) {
                ...
                if (s.period) {
                  ...
                  process.stdout.write(
                    'Waiting on first live trade to display reports, could be a few minutes ...'
                  )
                }
              }
            )
            ...
            if (
              ...
              if (prev_timeout) {
                console.error(
                  '\n' +
                    moment().format('YYYY-MM-DD HH:mm:ss') +
                    ' - getTrades request timed out. retrying...'
                )
              }
              prev_timeout = true
              ...
              console.error(
                '\n' +
                  moment().format('YYYY-MM-DD HH:mm:ss') +
                  ' - getTrades request failed. retrying...'
              )
              console.error(err)
            }
            return
        ...
        function saveTrade(trade) {
          ...
          marker.to = marker.to
            ? Math.max(marker.to, trade_cursor)
            : trade_cursor
