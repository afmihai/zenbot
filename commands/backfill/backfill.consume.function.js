...
*/
const tradesService = require('../../lib/services/trades-service')

module.exports = function (conf) {
  const tradesServiceInstance = tradesService(conf)

  return (mostRecentlyProcessedTradeId, queue, cb) => {
    tradesServiceInstance
      .getTrades(mostRecentlyProcessedTradeId)
      .then(function (returnedTrades) {
        if (returnedTrades.length > 0) {
          ...
          cb(
            null,
            'cp_process',
            returnedTrades[returnedTrades.length - 1].trade_id
          )
        } else {
