...
module.exports = function vma(s, key, length, source_key) {
  ...
  if (s.lookback[0] != undefined) {
    ...
    let pdmS = (s.period['pdmS'] =
      k * pdm +
      (s.lookback[0]['pdmS'] != undefined
        ? s.lookback[0]['pdmS'] * (1 - k)
        : 0))
    let mdmS = (s.period['mdmS'] =
      k * mdm +
      (s.lookback[0]['mdmS'] != undefined
        ? s.lookback[0]['mdmS'] * (1 - k)
        : 0))
    let s0 = pdmS + mdmS
    let pdi = pdmS / s0
    let mdi = mdmS / s0
    let pdiS = (s.period['pdiS'] =
      k * pdi +
      (s.lookback[0]['pdiS'] != undefined
        ? s.lookback[0]['pdiS'] * (1 - k)
        : 0))
    let mdiS = (s.period['mdiS'] =
      k * mdi +
      (s.lookback[0]['mdiS'] != undefined
        ? s.lookback[0]['mdiS'] * (1 - k)
        : 0))
    let d = Math.abs(pdiS - mdiS)
    let s1 = pdiS + mdiS
    iS = s.period['iS'] =
      (k * d) / s1 +
      (s.lookback[0]['iS'] != undefined ? s.lookback[0]['iS'] * (1 - k) : 0)
  }
  if (s.lookback.length > length) {
    ...
    let vma = (s.period['vma'] =
      k * vI * s.period[source_key] +
      (s.lookback[0]['vma'] != undefined
        ? s.lookback[0]['vma'] * (1 - k * vI)
        : 0))
    s.period[key] = vma
  }
}
