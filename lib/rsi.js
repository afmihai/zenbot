...
module.exports = function rsi(s, key, length) {
  if (s.lookback.length >= length) {
    const avg_gain = s.lookback[0][key + '_avg_gain']
    const avg_loss = s.lookback[0][key + '_avg_loss']
    if (typeof avg_gain === 'undefined') {
      ...
      const current_gain = s.period.close - s.lookback[0].close
      s.period[key + '_avg_gain'] =
        (avg_gain * (length - 1) + (current_gain > 0 ? current_gain : 0)) /
        length
      const current_loss = s.lookback[0].close - s.period.close
      s.period[key + '_avg_loss'] =
