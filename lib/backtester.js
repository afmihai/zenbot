...
const Phenotypes = require('./phenotype')
...
let processOutput = function (output, taskStrategyName, pheno) {
  ...
  if (
    ...
    typeof simulationResults.simresults !== typeof undefined
  ) {
    ...
    wins =
      simulationResults.simresults.total_sells -
      simulationResults.simresults.total_losses
    ...
    console.log(
      `Couldn't find simulationResults for ${pheno.backtester_generation}`
    )
...
let monitor = {
  ...
  reportStatus: function () {
        ...
        if (Object.keys(p.sim).length === 0) {
          ...
          if (p.command) {
            ...
            if (bestP == null || bestBalance < balance) {
              ...
              bestP &&
              bestBalance == balance &&
              bestP.command.iteration > p.command.iteration
            ) {
    ...
    inProgress.forEach(function (p) {
      ...
      var currentTime
      if (c.currentTimeString)
        currentTime = moment(c.currentTimeString, 'YYYY-MM-DD HH:mm:ss')
      if (currentTime && currentTime.isBefore(c.queryStart))
        c.queryStart = currentTime
      ...
      if (currentTime && c.queryStart && c.queryEnd) {
        ...
        var eta = c.startTime
          .clone()
          .add(timeElapsed / percentage, 'milliseconds')
        ...
        if (homeStretchMode)
          inProgressStr.push(
            `${(c.iteration + ':').gray} ${(percentage * 100).toFixed(
              1
            )}% ETA: ${monitor.distanceOfTimeInWords(eta, now)}`
          )
        else
          inProgressStr.push(
            `${(c.iteration + ':').gray} ${(percentage * 100).toFixed(1)}%`
          )
      }
    })
    ...
    process.stdout.write(
      `Done: ${simsDone.toString().green}, Active: ${
        simsActive.toString().yellow
      }, Remaining: ${simsRemaining.toString().gray}, `
    )
    if (simsErrored > 0)
      process.stdout.write(`Errored: ${simsErrored.toString().red}, `)

    process.stdout.write(
      `Completion: ${z(5, n(percentage).format('0.0') + '%', ' ').green} `
    )
    ...
    let bestBalanceString = z(5, n(bestBalance || 0).format('0.0000'), ' ')[
      bestBColor
    ]
    process.stdout.write(
      `Best Balance(${
        (bestP ? bestP.command.iteration.toString() : '?')[bestBColor]
      }): ${bestBalanceString}`
    )
  ...
  stop: function (label) {
    ...
    var timeStr = this.distanceOfTimeInWords(
  ...
  runCommand: (taskStrategyName, phenotype, command, cb) => {
    ...
    proc.on('exit', () => {
      ...
      try {
        ...
        console.log(
          `Bad output detected on sim ${command.iteration} while running:`
        )
