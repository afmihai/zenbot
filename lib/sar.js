module.exports = function sar(s, key, initAccel, deltaAccel, accelMax) {
  ...
  if (s.period[key + '_sar#'] < 0) {
    ...
    if (s.period[key + '_sar#'] == 1) {
      ...
      s.period[key + '_ep'] = Math.max(
        s.period.high,
        s.lookback[0][key + '_ep']
      )
    }
  }
  ...
  if (Math.abs(s.period[key + '_sar#']) == 1) {
    ...
    s.period[key + '_af'] = Math.min(
      accelMax,
      deltaAccel + s.lookback[0][key + '_af']
    )
  }
  ...
  if (s.lookback[0][key + '_sar#'] < 0) {
    s.period[key + '_tsar'] = Math.max(
      s.lookback[0][key] +
        s.lookback[0][key + '_af'] *
          (s.lookback[0][key + '_ep'] - s.lookback[0][key]),
      s.lookback[0].high,
      s.lookback[1].high
    )
  } else {
