...
module.exports = function (conf) {
  ...
  function _isWithinRange(obj) {
    const id = obj.trade_id

    const record = ranges.find(record => {
  ...
  function _merge() {
    let rtn = false // true when we have gone through the list, and done one merge. Indicates another merge might be fruitful.
  ...
  theService.ping = obj => {
    let theFarthestIdInRange = -1

    let rtn = _isWithinRange(obj)
    if (rtn !== undefined) {
        ...
        .deleteMany(
          ...
          function (/*err, data*/) {
            if (ranges.length > 1) {
              collectionServiceInstance
                .getResumeMarkers()
                .insertMany(ranges, function (err /*, data*/) {
                  if (err) throw err
              ...
              collectionServiceInstance
                .getResumeMarkers()
                .insertOne(ranges[0], function (err /*, data*/) {
                  if (err) throw err
