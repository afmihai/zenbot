...
module.exports = function (conf) {
    ...
    return new Promise(function (resolve /*, reject*/) {
        ...
        .toArray(function (err, data) {
          if (data.length === 0) {
            ...
            if (exchangeServiceInstance.getExchange().historyScanUsesTime) {
              const id = _getFromOrToFromExchangeAttributes(exchangeAttributes)
              ...
              if (id) {
                ...
                const selectorNormalized = exchangeServiceInstance.getSelector()
                  .normalized
                ...
                collectionServiceInstance
                  ...
                  .then(result => {
                    _setFromOrToOnExchangeAttributesToTime(
                      result,
                      exchangeAttributes
                    )
      ...
      function resolveUsingExchangeServiceGetTrades() {
        exchangeServiceInstance
          .getExchange()
          .getTrades(exchangeAttributes, function (err, results) {
            if (err) throw err
  ...
  function _getInitialQueryAttributes(tradeId) {
    ...
    const selectorNormalized = exchangeServiceInstance.getSelector().normalized
  ...
  function _getInitialExchangeAttributes(tradeId) {
    ...
    q.product_id =
      exchangeServiceInstance.getSelector().asset +
      '-' +
      exchangeServiceInstance.getSelector().currency

    if (tradeId !== undefined) {
      if (
        exchangeServiceInstance.getExchange().historyScan ===
        exchangeServiceInstance.BACKWARD
      ) {
        q.to = tradeId
      } else {
        q.from = tradeId
