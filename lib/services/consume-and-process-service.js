...
module.exports = function (conf) {
  ...
  let onConsumeFunc
  let onProcessFunc
  let afterOnConsumeFunc
  let afterOnProcessFunc

  const Queue = require('../_data-structures/Queue.js')
  ...
  theService.go = targetTimeInMillis => {
    const rtn = new Promise(resolve => {
      const resumeMarkerService = resumeMarkersService(conf)
      ...
      _emitter.on('cp_consume', record => {
        onConsumeFunc(
          record,
          _queue,
          (err, rtnCode, mostRecentlyProcessedRecordId) => {
