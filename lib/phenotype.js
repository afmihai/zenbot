...
module.exports = {
  create: function (strategy) {
    ...
    for (let k in strategy) {
      ...
      if (v.type === 'int') {
        ...
        r[k] = (
          Math.floor((Math.random() * (v.max - v.min + v.factor)) / v.factor) *
            v.factor +
          v.min
        ).toFixed(decimals)
  ...
  mutation: function (oldPhenotype, strategy) {
    const r = module.exports.create(strategy)
  ...
  crossover: function (phenotypeA, phenotypeB, strategy) {
    ...
    for (let k in strategy) {
      ...
      p1[k] =
        Math.random() <= PROPERTY_CROSSOVER_CHANCE
          ? phenotypeA[k]
          : phenotypeB[k]
      p2[k] =
        Math.random() <= PROPERTY_CROSSOVER_CHANCE
  ...
  fitness: function (phenotype) {
    ...
    if (phenotype.fitnessCalcType == 'profitwl') {
      let profit =
        phenotype.sim.profit +
        phenotype.sim.assetCapital * phenotype.sim.lastAssestValue
      ...
      if (phenotype.minTrades > 0) {
        if (phenotype.sim.wins < phenotype.minTrades && phenotype.sim.wins == 0)
          return 0.0
        if (phenotype.sim.wins < phenotype.minTrades)
          return (phenotype.sim.wins / phenotype.minTrades + profit) / 100
      }
      ...
      if (phenotype.minTrades > 0) {
        if (phenotype.sim.wins < phenotype.minTrades && phenotype.sim.wins == 0)
          return 0.0
        if (phenotype.sim.wins < phenotype.minTrades)
          return (phenotype.minTrades + profit) / 1000
      }
    ...
    if (phenotype.fitnessCalcType == 'wl') {
      ...
      if (phenotype.minTrades > 0) {
        if (phenotype.sim.wins < phenotype.minTrades && phenotype.sim.wins == 0)
          return 0.0
        if (phenotype.sim.wins < phenotype.minTrades)
          return phenotype.sim.wins / phenotype.minTrades / 100
      }
      let wlRatio = phenotype.sim.wins / phenotype.sim.losses
      if (isNaN(wlRatio)) {
        // zero trades will result in 0/0 which is NaN
      ...
      if (phenotype.minTrades > 0) {
        ...
        if (phenotype.sim.wins < phenotype.minTrades)
          return (
            (phenotype.sim.wins / phenotype.minTrades + vsBuyHoldRate) / 100
          )
      }
      let wlRatio = phenotype.sim.wins / phenotype.sim.losses
      if (isNaN(wlRatio)) {
  ...
  competition: function (phenotypeA, phenotypeB) {
    ...
    return (
      module.exports.fitness(phenotypeA) >= module.exports.fitness(phenotypeB)
    )
  },
