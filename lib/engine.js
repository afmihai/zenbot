...
const { formatAsset, formatPercent, formatCurrency } = require('./format')
...
module.exports = function (s, conf) {
  ...
  function syncBalance(cb) {
    ...
    s.exchange.getBalance({ currency: s.currency, asset: s.asset }, function (
      err,
      balance
    ) {
  ...
  function cancelOrder(order, type, do_reorder, cb) {
    s.exchange.cancelOrder(
      { order_id: order.order_id, product_id: s.product_id },
      function () {
