...
describe('Trades Service', function () {
  ...
  describe('getTrades when DB returns nothing, and API returns two trades', function () {
    it('calls getTrades correctly', function (done) {
      var instance = service({})
      var normalizedSelector = exchangeServiceFactory({}).getSelector()
        .normalized
  ...
  describe('getTrades when a tradeId is passed in, but the DB returns no results, and exchange is forward, and its history scan uses time', function () {
    var getTradesOptionsObservingFuncSpy = jasmine.createSpy(
      'getTradesOptionsObservingFunc'
    )
    ...
    it('calls the DB to get the trade which has the time which is then passed to the exchange, and returns two valid zenbot trades', function (done) {
      var conf = {
        ...
        tradesArray: [],
        exchangeTradesArray: [
          { trade_id: 3000, time: 99994 },
          { trade_id: 3001, time: 99992 },
        ],
        findOneReturnTrade: {
      ...
      var normalizedSelector = exchangeServiceFactory(conf).getSelector()
        .normalized

      instance.getTrades(3000).then(data => {
        expect(getTradesOptionsObservingFuncSpy).toHaveBeenCalledWith({
          product_id: 'BTC-USD',
          from: 99992,
        })
        expect(mockFindOneFunction).toHaveBeenCalledWith({
          id: 'stub.BTC-USD-3000',
        })
