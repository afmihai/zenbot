...
module.exports = opts => {
  ...
  if (opts.tradesArray !== undefined && opts.tradesArray !== null)
    ...
    tradesArray = [
      {
        id: 'stub.BTC-USD-3000',
        trade_id: 3000,
        time: 99992,
        selector: 'stub.BTC-USD',
      },
      {
        id: 'stub.BTC-USD-3001',
        trade_id: 3001,
        time: 99994,
        selector: 'stub.BTC-USD',
      },
  ...
  if (opts.resumeMarkersArray !== undefined && opts.resumeMarkersArray !== null)
    ...
    resumeMarkersArray = [
      { from: 2994, to: 2998, oldest_time: 99960, newest_time: 99986 },
    ]
  ...
  if (
    opts.mockDeleteManyFunction !== undefined &&
    opts.mockDeleteManyFunction !== null
  )
    mockDeleteManyFunction = opts.mockDeleteManyFunction

  var mockInsertManyFunction
  if (
    opts.mockInsertManyFunction !== undefined &&
    opts.mockInsertManyFunction !== null
  )
    mockInsertManyFunction = opts.mockInsertManyFunction

  var mockFindOneFunction
  if (
    opts.mockFindOneFunction !== undefined &&
    opts.mockFindOneFunction !== null
  )
  ...
  var rtn = {
    getTrades: () => {
      return {
        findOne: query => {
          if (mockFindOneFunction) mockFindOneFunction(query)
        ...
        insert: trade => {
          return {
            then: (cb /*, err*/) => {
              // TODO: should this be (err, cb) instead?
              cb(trade)
            },
          }
        },
    ...
    getResumeMarkers: () => {
      return {
        ...
        update: (opts, doc, func) => {
          func(null, null)
          if (mockUpdateFunction) mockUpdateFunction(opts, doc, func)
        },
        deleteMany: (q, cb) => {
          cb()
          if (mockDeleteManyFunction) mockDeleteManyFunction(q)
        },
        insertMany: (q, cb) => {
          cb()
          if (mockInsertManyFunction) mockInsertManyFunction(q)
        },
      }
    },
  } // collection service
